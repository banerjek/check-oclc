...
foreach $entry(@mildata) {
	...
	if ($fields[2] =~ /[a-z]/) {
		...
		if ($content !~ /<datafield ind1="." ind2="." tag="210">/) {
			$record_message .= "Key title not found. ";
			}
		}

	###################################################################
	# this field is split into an array since it could contain 0, 1, or 
	# many entries. The list used to generate the file makes sure there
	# are always subjects, so we don't need to see if they exist
	#
	# If you create other repeating fields that you want to interate 
	# through, you'll need to declare the arrays as well as the variables
	# you use to iterate through them with above
	###################################################################
	@subjects = split(/;/, $fields[1]);

	#################################################
	# Let's see if they even exist in the OCLC record.
	# Otherwise, we check to see if any that we know
	# about are missing from the master record
	#################################################
	if ($content !~ /<datafield ind1="." ind2="2" tag="650">/) {
			$record_message .= "No MeSH headings found. ";
			} else {
			foreach $subject(@subjects) {
				
				###############################################################################
				# The \Q tells to quote the string so that the $ doesn't simply take its normal 
				# value as part of a regular expression
				###############################################################################
				if ($content !~ /<datafield ind1="." ind2="2" tag="650"><subfield code="a">\Q$subject/) {
					$record_message .= "MeSH heading $subject missing. ";
					}
				}

		}	

	print $oclc . "\t" . $record_message . "\n";

	if (length($record_message) > 1) {
		##########################################################################
		# Output file includes a link to staff view to facilitate updating records
		##########################################################################
		print OUTFILE $oclc . "\t" . $record_message . "\t" . 'http://catalogs.ohsu.edu/search/o' . $oclc . '/o' . $oclc . '/1%2C1%2C1%2CE/marc&FF=o' . $oclc . "\n";
		}
